...
// 1. DATA PRODUK (HARUS SAMA PERSIS dengan file lain)
$products_data = [
...
$id_transaksi = time(); // Menggunakan timestamp sederhana sebagai ID
...
.item-row {
    display: grid;
    grid-template-columns: 1fr 50px 80px; /* Nama, Qty, Harga */
    gap: 5px;
    margin-bottom: 5px;
    font-size: 14px;
}
...
<div class="box">
    ...
    ?>
        <div class="item-row">
            ...
            <span style="text-align: right;">Rp <?= number_format($subtotal) ?></span>
        </div>
    <?php 
        endif;
    endforeach; 
