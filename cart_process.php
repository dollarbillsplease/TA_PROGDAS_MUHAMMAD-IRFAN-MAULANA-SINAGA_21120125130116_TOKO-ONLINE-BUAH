...
// --- 1. DATA PRODUK (Disalin persis dari index.php) ---
// CATATAN: Indeks array (0, 1, 2, dst.) adalah ID produk yang digunakan.
$products_data = [
...
// Cek apakah ada data POST yang masuk
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // Ambil ID produk (ini adalah indeks array dari index.php)
    $product_index = isset($_POST['product_index']) ? (int)$_POST['product_index'] : null;
    // Ambil aksi dari cart.php (decrease/increase)
    $product_id = isset($_POST['product_id']) ? (int)$_POST['product_id'] : null;
    $action = $_POST['action'] ?? null;
    
    // Gunakan product_index atau product_id, keduanya harus berupa indeks array (0-14)
    $id_to_process = $product_index !== null ? $product_index : $product_id;

    // Pastikan ID valid dan ada di daftar produk
    if ($id_to_process !== null && array_key_exists($id_to_process, $products_data)) {
        
        // ==========================================================
        // A. LOGIKA TAMBAH DARI INDEX.PHP (Tombol Tambah ➕)
        // ==========================================================
        if (isset($_POST['add_to_cart'])) {
            // Menambahkan kuantitas ke keranjang menggunakan ID (indeks array)
        ...
        // ==========================================================
        // B. LOGIKA UBAH KUANTITAS DARI CART.PHP (Tombol +/-)
        // ==========================================================
        if ($action) {
            ...
            if ($action === 'increase') {
                ...
                if (isset($_SESSION['cart'][$id_to_process])) {
                    ...
                    // Jika kuantitas menjadi 0, hapus dari keranjang
...
// Jika tidak ada aksi yang valid, kembali ke halaman utama
header("Location: index.php");
exit;
?>